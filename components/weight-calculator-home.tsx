...
export default function WeightCalculatorHome() {
  ...
  // Set up 1RM configuration on mount - using useRef to prevent infinite loops
  useEffect(() => {
  ...
  const calculateWeightsOnly = useCallback(() => {
    try {
      ...
      // Calculate weights (1RM-only, no mode parameter)
  ...
  const saveToHistory = useCallback(() => {
    try {
      ...
      calculator.addToHistory(weight, sets, reps, calculatedWeights, calculator.selectedScheme)
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      {calculator && (
        <>
          {/* Bar weight and plate selection */}
          <div className="mb-6 space-y-3">
            <BarWeightSelector />
            <PlatePicker />
          </div>

          {/* 1RM weight and scheme inputs */}
          <PercentageInputs
            targetWeight={calculator.targetWeight}
