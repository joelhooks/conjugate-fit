...
export default function PlatePicker() {
  ...
  const renderCollapsedPlates = () => {
    ...
    if (sortedPlates.length === 0) {
      return (
        <div className="text-xs text-gray-400 py-2">No plates selected</div>
      );
    }
    const maxHeight = Math.max(
      ...sortedPlates.map((p) => PLATE_HEIGHTS[p] || 30),
      30,
    );
      ...
      <CollapsibleContent>
        <div className="p-4 bg-gray-800/80 border border-gray-700 border-t-0 rounded-b-lg space-y-3">
          ...
          <p className="text-[10px] text-gray-500 text-center">
            Tap a plate to enable/disable. Adjust quantity (pairs per side) or
            set to unlimited.
          </p>
        </div>
      </CollapsibleContent>
